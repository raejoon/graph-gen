...
python ../graph-generate/main.py --small --max 6 --outdir $GRAPH_DIR
...
do
    ...
    mkdir -p $ALPHA_DIR
    python ../graph-simulate/main.py --graph-dir $GRAPH_DIR \
        --outdir $ALPHA_DIR --seed-list $SEED_FILE --algo solo2 --alpha $ALPHA
    
    ANALYSIS_FILE=$ANALYSIS_DIR/solo-$ALPHA
    python ../graph-simulate/analyze.py --logdir $ALPHA_DIR --converge-time \
        --outfile $ANALYSIS_FILE

    CDF_FILE=$ANALYSIS_DIR/solo-$ALPHA-cdf
    python ../graph-simulate/analyze.py --logdir $ALPHA_DIR --converge-time \
        --cdf --outfile $CDF_FILE
